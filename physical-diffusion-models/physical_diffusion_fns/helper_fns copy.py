...
def _log_p_mixture(points, means, var_t,
                   chunk_means: int = 1024,
                   chunk_points: int | None = None):
    """
    Memory‑friendly log p_t(points) for an isotropic Gaussian mixture.

    points : (P, dim)
    means  : (M, dim)
    var_t  : scalar
    chunk_means  : slice width along mixture components  (static int)
    chunk_points : optional slice width along points     (static int or None)
    """
    P, dim = points.shape
    M      = means.shape[0]
    log_norm = -0.5 * dim * jnp.log(2.0 * jnp.pi * var_t)

    means_sq = jnp.sum(means ** 2, axis=1)                # (M,)

    # Build static point‑slice ranges (pure Python)
    if chunk_points is None:
        point_ranges = [(0, P)]
    else:
        point_ranges = [(i, min(P, i + chunk_points))
                        for i in range(0, P, chunk_points)]

    out_logs = []

    for p_start, p_end in point_ranges:
        p_len  = p_end - p_start
        pts    = lax.dynamic_slice_in_dim(points, p_start, p_len, axis=0)  # (p, dim)
        pts_sq = jnp.sum(pts ** 2, axis=1, keepdims=True)                  # (p,1)

        # running (max, sum_exp) state for log‑sum‑exp over mixture comps
        init  = (-jnp.inf * jnp.ones(p_len), jnp.zeros(p_len))

        def comp_body(carry, m_start):
            cur_max, cur_sum = carry

            # dynamic slice for means and its norm²
            m_slice = lax.dynamic_slice_in_dim(means, m_start,
                                               chunk_means, axis=0)        # (chunk, dim)
            ms_sq   = lax.dynamic_slice_in_dim(means_sq, m_start,
                                               chunk_means, axis=0)        # (chunk,)

            # (p, dim)·(dim, chunk) → (p, chunk)
            dots   = pts @ m_slice.T
            log_c  = log_norm - 0.5 / var_t * (
                        pts_sq + ms_sq[None, :] - 2.0 * dots
                     )                                                     # (p, chunk)
        ...
        (max_log, exp_sum), _ = lax.scan(
            comp_body, init, jnp.arange(0, M, chunk_means)
        )
        out_logs.append(max_log + jnp.log(exp_sum) - jnp.log(M))

    return jnp.concatenate(out_logs, axis=0)      
...
#     *, exact=True, oversample_factor=4, chunk_=1024
# ):
#     """
...
#     chunk_means=1024, chunk_points=None
# ):
#     """As before, but uses the new _log_p_mixture."""
#     key1, key2, key3 = jr.split(key, 3)
#     M, dim = samples0.shape
...
#     if k * var_t >= 2.0:
#         raise ValueError("k·var_t ≥ 2 → q_t not normalisable.")
...
#     # 2.3  importance weights   ( ***fixed*** )
#     log_w = 0.5 * k * jnp.sum(prop ** 2, axis=-1)      # no + log_p
#     log_w = log_w - jsp.special.logsumexp(log_w)
#     w     = jnp.exp(log_w)
...
def sample_total_distribution(
    t, n_samples, D, sigma_final, samples0, key, k,
    beta: float = 1.0,
    *,
    ...
    """
    Samples from q_t(x) ∝ p_t(x)^2 · exp(+½ k‖x‖²)
    using importance resampling with p_t as proposal.
    """
    key_prop, key_resample = jr.split(key)
    M, dim = samples0.shape
    ...
    var_t = D * sigma_final**2 * (1.0 - jnp.exp(-2*beta*t / sigma_final**2))
    if k * var_t >= 2.0:
        raise ValueError("k·var_t ≥ 2 ⇒ target not normalisable")

    # ------------------------------------------------------------------
    # 1) draw proposals  x ~ p_t
    # ------------------------------------------------------------------
    prop_N = oversample_factor * n_samples
    prop   = sample_forward_process(
        t, prop_N, D, sigma_final, samples0, key_prop, beta=beta
    )                                            # (prop_N, dim)

    # ------------------------------------------------------------------
    # 2) log p_t(x)     —  via memory–friendly mixture evaluator
    # ------------------------------------------------------------------
    means = a_t * samples0                       # (M, dim)
    log_p = _log_p_mixture(
        prop, means, var_t,
    ...
    # ------------------------------------------------------------------
    # 3) importance weights  w ∝  p_t(x) · exp(+½ k‖x‖²)
    # ------------------------------------------------------------------
    log_w = log_p + 0.5 * k * jnp.sum(prop**2, axis=-1)
    log_w = log_w - jsp.special.logsumexp(log_w)   # normalise safely
    w     = jnp.exp(log_w)

    # ------------------------------------------------------------------
    # 4) systematic (or multinomial) resampling
    # ------------------------------------------------------------------
    idx = jr.choice(
        key_resample,
        prop_N,
        shape=(n_samples,),
        p=w,           # replace=True keeps ESS high; set False if you prefer distinct draws
    )
    return prop[idx]
