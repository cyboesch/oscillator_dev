...
n_neighbour_couplings = 1  # Reduced from 8 to 4 for memory efficiency
...
# Original pixel range [0, 1] mapped through normalization: (x - mean) / std
...
def run_reverse_process_SDE(
    ...
    images_generated_flat = final_samples 
...
images_true_clipped = np.clip(np.array(samples_target_unscaled.reshape(-1, resolution[0], resolution[1])), clip_value_min, clip_value_max)
images_generated_clipped = np.clip(np.array(images_generated_non_smoothed_sde), clip_value_min, clip_value_max)
...
axes[0, 0].set_title(f"True images (range:[{clip_value_min:.4f},{clip_value_max:.4f}])", loc='left', fontsize=16)
axes[n_rows, 0].set_title(f"SDE sampled images (range:[{clip_value_min:.4f},{clip_value_max:.4f}]), rtol={rtol_sde}, atol={atol_sde}, brownian_tolerance={brownian_tolerance}", loc='left', fontsize=16)

plt.subplots_adjust(wspace=0.01, hspace=0.5)
plt.savefig(f'{plot_folder}/samples_{reverse_sde_suffix}_clipped_{clip_value_min:.4f}_to_{clip_value_max:.4f}.png')
plt.close()
