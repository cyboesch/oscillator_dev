...
training_method = "SM_at_kbT_minimal_memory" #"SM_at_kbT"
...
# Reverse initialization (physical): equilibrium chain at reverse time 0
...
equilibrium_dt0 = 1e-3           # initial dt for adaptive solver
equilibrium_rtol = 1e-3
equilibrium_atol = 1e-6

# Diagnostics: compare equilibrium marginals vs analytical forward marginals at t=t_forward
equilibrium_compare_n_forward = 2048
equilibrium_compare_dims = 16    # number of coordinates (pixels) to plot in comparison grid
...
def plot_forward_vs_equilibrium_marginals(
    samples_forward_tfinal,
    samples_equilibrium,
    dims,
    *,
    path,
    ...
    """Compare 1D marginals for selected dimensions."""
    import math
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    fwd = np.array(samples_forward_tfinal)
    eq = np.array(samples_equilibrium)

    for ax_idx, dim in enumerate(dims):
        ax = axes[ax_idx]
        ax.hist(fwd[:, dim], bins=bins, density=True, alpha=0.35, label="forward marginal")
        ax.hist(eq[:, dim], bins=bins, density=True, alpha=0.35, label="equilibrium chain")
        ax.set_title(f"dim {dim}", fontsize=fontsize)
    ...
    fig.suptitle("Forward marginal at t=t_forward vs equilibrium at reverse t=0", fontsize=fontsize + 2)
...
def plot_forward_vs_equilibrium_sample_grids(
    samples_forward_tfinal_scaled,
    samples_equilibrium_scaled,
    *,
    path,
    filename,
    n_show=40,
):
    """Save a grid of example samples (forward vs equilibrium) in pixel space."""
    import numpy as np
    import matplotlib.pyplot as plt

    n_show = int(n_show)
    n_show = max(10, (n_show // 10) * 10)  # multiple of 10

    fwd = np.array(samples_forward_tfinal_scaled)
    eq = np.array(samples_equilibrium_scaled)
    n_show = min(n_show, fwd.shape[0], eq.shape[0])

    # convert back to pixel space for visualization
    fwd_pixels = (fwd[:n_show] / additional_rescaling) * float(std_MNIST) + float(mean_MNIST)
    eq_pixels = (eq[:n_show] / additional_rescaling) * float(std_MNIST) + float(mean_MNIST)

    fwd_imgs = fwd_pixels.reshape(-1, resolution[0], resolution[1])
    eq_imgs = eq_pixels.reshape(-1, resolution[0], resolution[1])
    ...
    for idx in range(n_show):
        row = idx // 10
        col = idx % 10
        axes[row, col].imshow(fwd_imgs[idx], cmap="gray")
        axes[row, col].axis("off")

        axes[row + n_rows, col].imshow(eq_imgs[idx], cmap="gray")
        axes[row + n_rows, col].axis("off")

    axes[0, 0].set_title("Forward marginal samples at t=t_forward", loc="left", fontsize=14)
    axes[n_rows, 0].set_title("Equilibrium-chain samples (reverse t=0 params)", loc="left", fontsize=14)
...
def run_reverse_process_SDE(params_interpolator, suffix, key, Temp, atol, rtol, brownian_tolerance, ode_solve=False, t_final=None):
    ...
    # -------------------------------------------------------------------------
    # Initial conditions for reverse process
    # -------------------------------------------------------------------------
    if use_equilibrium_init:
        ...
        drift_eq, diffusion_eq = setup_overdamped_SDE(
            ...
            params_reverse_t0,
            N_osc,
            time_dependent_parms=False,
            Temp=Temp,
        )

        # start chain from a simple Gaussian (not from the forward marginal)
        key, subkey_init = jr.split(key)
        init_eq = jr.normal(subkey_init, (N_osc,)) * (jnp.sqrt(Temp) * sigma_forward)

        # run equilibrium chain and keep states (correlated); burn-in + thinning later
        t0_eq = 0.0
        t1_eq = float(t_equilibrium)
        print(f"Running equilibrium chain: t_equilibrium={t_equilibrium}, N_save={n_equilibrium_save}")
        key, subkey_brownian_eq = jr.split(key)
        sol_eq = solve_SDE(
            drift_eq,
            diffusion_eq,
            init_eq,
        ...
        chain = sol_eq.ys  # (N_save, N_osc)

        burnin_idx = int(equilibrium_burnin_frac * n_equilibrium_save)
        eq_samples = chain[burnin_idx::equilibrium_thinning]
        print(f"Equilibrium samples after burn-in/thinning: {eq_samples.shape[0]}")

        # Save equilibrium samples (scaled units)
        eq_path = os.path.join(output_dir, f"equilibrium_samples_{suffix}.npy")
        jnp.save(eq_path, eq_samples)
        print(f"Equilibrium samples saved to {eq_path}")

        # Diagnostics: compare marginals vs analytical forward marginal at t=t_forward
        key, subkey_forward_cmp = jr.split(key)
        samples_forward_tfinal = sample_forward_process(
            t_final,
            equilibrium_compare_n_forward,
            D=Temp,
            sigma_final=sigma_forward,
            samples0=samples_target,
            key=subkey_forward_cmp,
            beta=1.0,
        )

        # pick dimensions spread across the vector
        dims = jnp.linspace(0, N_osc - 1, equilibrium_compare_dims).astype(int).tolist()
        plot_forward_vs_equilibrium_marginals(
            samples_forward_tfinal,
            eq_samples,
            dims,
            path=plot_folder,
            filename=f"forward_vs_equilibrium_marginals_{suffix}.png",
            bins=50,
            fontsize=10,
        )
        plot_forward_vs_equilibrium_sample_grids(
            samples_forward_tfinal,
            eq_samples,
            path=plot_folder,
            filename=f"forward_vs_equilibrium_samples_{suffix}.png",
            n_show=min(40, equilibrium_compare_n_forward),
        )
        print(f"Saved forward-vs-equilibrium marginal comparison plot to {plot_folder}")

        # subsample equilibrium states for reverse trajectories
        key, subkey_choose = jr.split(key)
        n_available = int(eq_samples.shape[0])
        replace = n_available < n_trajectories
        idx = jr.choice(subkey_choose, n_available, (n_trajectories,), replace=replace)
        initial_states = eq_samples[idx]

        # cleanup (keep initial_states)
        del sol_eq, chain, eq_samples, samples_forward_tfinal
        gc.collect()
        print("Equilibrium initialization complete; starting reverse trajectories...")
    else:
        print("Initializing reverse trajectories from analytical forward marginal (legacy/cheating init)...")
