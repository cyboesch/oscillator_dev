...
# Exact Gaussian terminal energy parameters (same parameterization as energy_fn)
# Stationary distribution for overdamped Langevin with this E at temperature Temp is:
#   p(x) ∝ exp(-E(x)/Temp) = exp(-||x||^2 / (2*Temp*sigma_forward^2))
# i.e. N(0, Temp*sigma_forward^2 I)
params_gaussian_terminal = jnp.zeros_like(params_flattened_initial)
params_gaussian_terminal = params_gaussian_terminal.at[0:N_osc].set(1.0 / (sigma_forward**2))
...
def run_reverse_process_SDE(params_interpolator, suffix, key, Temp, atol, rtol, brownian_tolerance, ode_solve=False, t_final=None):
    ...
    if ode_solve:
        params_interpolator_reverse_plus_linear = lambda t: Temp*params_interpolator_with_analytic_end(t_final - t) - 1 / sigma_forward**2 * forward_params
    elif training_method.startswith("SM_at_kbT"):
        params_interpolator_reverse_plus_linear = lambda t: 2*params_interpolator_with_analytic_end(t_final - t) - 1 / sigma_forward**2 * forward_params
    else:
        params_interpolator_reverse_plus_linear = lambda t: 2*Temp*params_interpolator_with_analytic_end(t_final - t) - 1 / sigma_forward**2 * forward_params
    ...
    key, subkey = jr.split(key)
    # Start reverse from the terminal Gaussian N(0, Temp*sigma_forward^2 I)
    initial_states = jr.normal(subkey, (n_trajectories, N_osc)) * (jnp.sqrt(Temp) * sigma_forward)
    key, subkey = jr.split(key)
    keys_brownian = jr.split(subkey, n_trajectories)
