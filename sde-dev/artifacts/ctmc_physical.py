...
class ContinuousTimeMarkovChain(eqx.Module):
    ...
    damping: Callable[[RealScalarLike, Y, Args], ArrayLike]  # Damping in Fox et al.
    ...
    def __init__(
        ...
        damping_fn: Union[Callable[[RealScalarLike, Y, Args], ArrayLike], None] = None,
        ...
        if damping_fn is None:
            self.damping = lambda t, z, args: jnp.zeros((z.shape[-1], z.shape[-1]))
        else:
            self.damping = damping_fn
    ...
    def drift(self, t: RealScalarLike, z: Y, args: Args) -> ArrayLike:
        ...
        DQ = jnp.add(self.damping(t, z, args), self.Q(t, z, args))
