...
class SimulationConfig:
    ...
    def create(cls, init_seed: int = 0):
        return cls(
            init_seed=init_seed,
            init_config=InitConfig(
                N=1_000_000, rng=jr.PRNGKey(init_seed), step_size=0.01
            ),
            cld_config=CLDConfig(
                state_dim=1,
                beta=0.1,
...
init_x0s_p0s = jax.device_put(
    vmap(sample_x0_p0)(jr.split(config.init_config.rng, config.init_config.N)),
    cpu_devices[0],
)  # (n_samples, 2)
...
def ItoMilstein_step(t, y):
    next_t = t + step_size
    next_y = solver.step(sde_terms, t, next_t, y, None, solver_state, made_jump=False)[
        0
    ]
...
# Run the simulation
# get initial conditions from init_x0s_p0s
n_inits = 100
idxs = jrnd.choice(
    jrnd.PRNGKey(0), jnp.arange(len(init_x0s_p0s)), (n_inits,), replace=False
)
_, _, full_trajectories, _ = vmap(
    lambda init_x0_p0: simulation_loop((0.0, init_x0_p0, trajectory, 0))
)(init_x0s_p0s[idxs])

full_trajectories = jax.device_get(full_trajectories)  # (n_inits, n_steps, state_dim)

# %%
# A_vv*(mu_v(t)-v(t))
# Assuming HSM
# Generate time array
ts = xjnp.linspace(0, t_final, num_steps)

n_subsample = 1000
idxs = np.random.choice(np.arange(config.cld_config.state_dim), n_subsample)
ys_to_plot = full_trajectories[:, :, idxs]
vs = ys_to_plot[:, :, 1]
xs = ys_to_plot[:, :, 0]
xs_to_plot = xs[:, :, idxs]
vs_to_plot = vs[:, :, idxs]
...
Sigma_inv_xxs = map(lambda t: cld.Sigma_inv_xx_t(t, **HSM_cov))(ts)
Sigma_inv_vvs = map(lambda t: cld.Sigma_inv_vv_t(t, **HSM_cov))(ts)
Sigma_inv_xvs = map(lambda t: cld.Sigma_inv_xv_t(t, **HSM_cov))(ts)
# map over inits also
mu_vs = map(lambda t: cld.mean(t, **HSM_mean))(ts)[:, 1]
mu_xs = map(lambda t: cld.mean(t, **HSM_mean))(ts)[:, 0]
...
norms_vv = map(
    lambda init: vmap(
        lambda t: jnp.abs(
            Sigma_inv_vvs.at[t].get() * (mu_vs.at[t].get() - init.at[t].get())
        )
    )(jnp.arange(len(ts)))
)(vs)
...
norms_xx = map(
    lambda init: vmap(
        lambda t: jnp.abs(
            Sigma_inv_xxs.at[t].get() * (mu_xs.at[t].get() - init.at[t].get())
        )
    )(jnp.arange(len(ts)))
)(xs)
...
norms_xv = map(
    lambda init: vmap(
        lambda t: jnp.abs(
            Sigma_inv_xvs.at[t].get() * (mu_vs.at[t].get() - init.at[t].get())
        )
    )(jnp.arange(len(ts)))
)(xs)

# Compute mean across initial conditions
mean_norms_vv = jnp.mean(norms_vv, axis=0)
mean_norms_xx = jnp.mean(norms_xx, axis=0)
mean_norms_xv = jnp.mean(norms_xv, axis=0)
# %%
# Use these mean norms for plotting and further calculations
...
norms_vv_to_plot = norms_vv_to_plot
norms_xx_to_plot = norms_xx_to_plot
norms_xv_to_plot = norms_xv_to_plot
...
fig.suptitle(
    ...
    + r"$\beta = $"
    + f"{config.cld_config.beta}, "
    + r"$\gamma = $"
    + f"{config.cld_config.gamma}",
    fontsize=16,
    y=1.02,
)
...
ax_dict["G"].plot(
    ts_to_plot, norms_xv_to_plot, label=r"norms $xv$", color=x_color, alpha=1.0
)
ax_dict["G"].plot(
    ts_to_plot, norms_vv_to_plot, label=r"norms $vv$", color=p_color, alpha=1.0
)
ax_dict["G"].set_title(
    r"Rolling Mean of Ratios $r_{\frac{vv}{xx}} = $"
    + r"$\frac{\|A_{vv}(t)(\mu_{v}(t) - v(t))\|}{\|A_{xx}(t)(\mu_{x}(t) - x(t))\|}$"
    + " and "
    + r"$r_{\frac{vv}{xv}} = $"
    + r"$\frac{\|A_{vv}(t)(\mu_{v}(t) - v(t))\|}{\|A_{xv}(t)(\mu_{x}(t) - x(t))\|}$"
    + "\n"
    + times_info_str,
    fontsize=14,
    y=1.05,
