...
class ContinuousTimeMarkovChain(eqx.Module):
    ...
    def __init__(
        ...
        D_fn: Union[Callable[[Y, Args], ArrayLike], None] = None,
        Q_fn: Union[Callable[[Y, Args], ArrayLike], None] = None,
        Gamma_fn: Union[Callable[[Y, Args], ArrayLike], None] = None,
    ):
        self.H = H_fn
        if D_fn is None:
    ...
    def Gamma(self, z: Y, args: Args):
        ...
        def partial_sum(i):
            return jnp.sum(
                jax.jacfwd(
                    lambda z, args: self.D(z, args)[i, :] + self.Q(z, args)[i, :]
                )(z, args)
            )
...
def create_ctmc_from_logdensity(
    ...
    H = lambda z, args: -1.0 * logdensity_fn(z, args)
