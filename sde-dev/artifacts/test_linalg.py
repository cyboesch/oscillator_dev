...
@jit
def get_inv_xx(S_xx, S_vv, S_xv):
    S_xx_inv = 1 / S_xx
    S_schur = S_vv - S_xv**2 * S_xx_inv
    S_schur_inv = 1 / S_schur
    return S_xx_inv + S_xx_inv**2 * S_xv**2 * S_schur_inv

@jit
def get_inv_vv(S_xx, S_vv, S_xv):
    S_xx_inv = 1 / S_xx
    S_schur = S_vv - S_xv**2 * S_xx_inv
    return 1 / S_schur

@jit
def get_inv_xv(S_xx, S_vv, S_xv):
    S_xx_inv = 1 / S_xx
    S_schur = S_vv - S_xv**2 * S_xx_inv
    S_schur_inv = 1 / S_schur
    return -S_schur_inv * S_xv * S_xx_inv
...
def test_get_specific_blocks(S_xx, S_vv, S_xv):
    # Compute inverse using numpy
    S = np.array([[S_xx, S_xv], [S_xv, S_vv]])
    S_inv_np = np.linalg.inv(S)
    ...
    # Compute specific blocks using our jitted functions
    S_inv_xx_jit = get_inv_xx(S_xx, S_vv, S_xv)
    S_inv_vv_jit = get_inv_vv(S_xx, S_vv, S_xv)
    S_inv_xv_jit = get_inv_xv(S_xx, S_vv, S_xv)
    
    # Assert that our results match numpy's results
    np.testing.assert_allclose(S_inv_xx_jit, S_inv_xx_np, rtol=1e-5)
    np.testing.assert_allclose(S_inv_vv_jit, S_inv_vv_np, rtol=1e-5)
    np.testing.assert_allclose(S_inv_xv_jit, S_inv_xv_np, rtol=1e-5)
