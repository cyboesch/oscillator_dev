...
from sde import sample as sample_SRA1
...
k_lin = jnp.array([141.1403, -68.54974, 14.409757])
k_duff = jnp.array([-38.744267, 1.9867308, -1.752297])
c_lin = jnp.array([52.788277, 1.535546])
c_optomech = jnp.array([0.84106606, -2.0334804])
# # Duffing params
m = jnp.array([1.0, 1.0])
...
for i in range(num_iterations + 1):
    ...
    for l, param in enumerate(opt_params):
        ...
        opt_params[l] = jnp.maximum(new_param, 1.0)
...
prob_from_energy_vals_fn = vmap(lambda x: jnp.exp(-energy_network_marg(x, k_lin_opt, k_duff_opt, c_lin_opt, c_optomech_opt, connectivity))) #(X1.
...
norm_const_opt = jnp.sum(inegrator_norm(prob_from_energy_vals_fn))

prob_from_energy_normalized = prob_from_energy_vals/norm_const_opt


# %%
jnp.sum(norm_const_opt)
...
sde_samples = sample_SRA1(energy_network_marg, jnp.zeros((2*N,)), (k_lin_opt, k_duff_opt), (c_lin_opt, c_optomech_opt), {'diag_B': 0.5*jnp.ones((N,)), 'diag_M_inv': jnp.ones((N,)), 't0': 0.0, 't1': 100.0, 'num_samples': 1000})
