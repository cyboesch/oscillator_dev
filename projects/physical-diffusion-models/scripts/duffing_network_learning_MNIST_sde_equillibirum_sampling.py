...
def run_equillibrium_sampling_SDE(key, Temp, atol, rtol, sim_time=100):    # Setup SDE functions
    drift_fn, diffusion_fn = setup_overdamped_SDE(energy_fn, params_history_all_t[0], N_osc, time_dependent_parms=False, Temp=Temp)
    ...
    reverse_trajectories_path = f"{output_dir}/equillibrium_sampling_trajectories.npy"
...
key, subkey = jr.split(key)
images_generated_non_smoothed_sde = run_equillibrium_sampling_SDE(subkey, Temp, atol_sde, rtol_sde, sim_time=10)
...
#####################################
# Plot histogram evolution for a subset of DOFs
#####################################
# Select a subset of DOFs to visualize (e.g., 4 DOFs)
n_dofs_to_plot = 4
selected_dofs = jnp.linspace(0, N_osc-1, n_dofs_to_plot, dtype=int)

# Create a figure with subplots for each selected DOF
fig, axes = plt.subplots(2, 2, figsize=(15, 10))
axes = axes.flatten()

# Number of time points to show (to avoid too many frames)
n_time_points = 20
time_indices = jnp.linspace(0, all_trajectories.shape[1]-1, n_time_points, dtype=int)

# Create animation frames
for t_idx in time_indices:
    for i, dof_idx in enumerate(selected_dofs):
        ax = axes[i]
        ax.clear()
        
        # Get values for this DOF at this time point
        values = all_trajectories[:, t_idx, dof_idx]
        
        # Plot histogram
        ax.hist(values, bins=30, density=True, alpha=0.7)
        ax.set_title(f'DOF {dof_idx} at t={ts[t_idx]:.2f}')
        ax.set_xlabel('Value')
        ax.set_ylabel('Density')
        
        # Set consistent x and y limits for better visualization
        ax.set_xlim(jnp.min(all_trajectories[:, :, dof_idx]), 
                   jnp.max(all_trajectories[:, :, dof_idx]))
        ax.set_ylim(0, 1.2 * jnp.max([ax.get_ylim()[1] for ax in axes]))
    
    plt.tight_layout()
    plt.pause(0.1)  # Pause briefly to see the frame

plt.savefig(f'{plot_folder}/histogram_evolution_selected_dofs.png')
plt.show()
