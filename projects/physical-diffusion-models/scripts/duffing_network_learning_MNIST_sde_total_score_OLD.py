...
problem_type_folder = f"MNIST_generation/total_score/Energy_fn_type_{energy_fn_type}_n_neighbour_couplings_{n_neighbour_couplings}_Temp_{Temp}"
...
if start_t_idx < len(forward_time_pts):
    ...
    plot_forward_marginals(samples_t, t_forward, sigma_forward, Temp=Temp, path=plot_folder, save_fig=True, fontsize=16)
...
def run_reverse_process_SDE(params_interpolator, suffix,key, Temp, atol, rtol, ode_solve=False):
    ...
    params_interpolator_reverse_plus_linear = lambda t: Temp*params_interpolator(t_forward - t)
    ...
    key, subkey = jr.split(key)
    initial_states = sample_total_distribution(t_forward, n_trajectories, sigma_final=sigma_forward, D=Temp, samples0=samples_target, key=subkey, k=1, beta= 1)
